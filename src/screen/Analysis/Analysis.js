...
const Analysis = ({ navigation }) => {
  ...
  const getData = async () => {
    ...
    if (expense && expense.length > 0) {
      expenseArr = expense.filter((exfilter) => {
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <TopnavBar />
        <HistoryCom month={month} setMonth={(month) => setMonth(month)} expenseTotal={expenseTotal} incomeTotal={incomeTotal} incomePress={() => setInSelect('income')} expensePress={() => setInSelect('expense')} />
        {
          addData == '' ? null :
        ...
        }
        {
          addData == '' ? <View style={styles.noRecors}>
            ...
            :
            <FlatList data={addData} contentContainerStyle={{ borderTopColor: colors.backColor, borderTopWidth: 1, paddingVertical: 10 }}
                    ...
                    <Swipeable
                      ...
                      <View style={styles.dataRecord}>
                        <View style={{ flexDirection: "row", alignItems: "center" }}>
                          ...
                          <Text style={{ marginRight: 20, width: 120, color: "#000" }}>{item.category ? item.category.title : ''}</Text>
                        </View>
