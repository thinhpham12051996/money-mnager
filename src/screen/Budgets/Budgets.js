...
import { incomeCategories, expenseCategories, incomeIconeHandler } from '../../constants/dataCom';
import { FlatList } from 'react-native-gesture-handler';
import { colors } from '../../constants/colors';
import moment from 'moment';
import { useFocusEffect } from '@react-navigation/native';
import { incomeTotalHandler, expenseTotalHandler } from '../../constants/functionCom';
...
const Budgets = ({ navigation }) => {
  ...
  const getData = async () => {
    ...
    var incomeArr = [], expenseArr = []
    var total = 0
    if (income && income.length > 0) {
      incomeArr = income.filter((incfilter) => {
        return moment(incfilter.date).isSame(month, 'month')
      }).map((inc) => {
        total += parseFloat(inc.amount)
        const category = incomeCategories.find(item => item?.id === inc?.categoryId)
    ...
    if (expense && expense.length > 0) {
      expenseArr = expense.filter((incfilter) => {
        return moment(incfilter.date).isSame(month, 'month')
      }).map((inc) => {
        ...
        var eLimit = limit.find((lim) => {
          ...
          return inc?.categoryId === lim?.categoryId && expenseMonth === LimitMonth
        })
        if (eLimit && eLimit.limit) inc.limit = eLimit.limit
        const category = expenseCategories.find(item => item?.id === inc?.categoryId)
        if (category) inc.category = category
        return inc
      })
    } else setAddData([])
  ...
  const dateHandler = (item, index) => {
    if (index == 0 || (index !== 0 && !moment(item?.date).isSame(addData?.[index - 1]?.date, 'day')))
      return (
        <Text style={{ marginHorizontal: 10, marginTop: 10 }}>{moment(item.date).format('DD-MM-YYYY')}</Text>
      )
    return null
  }
  ...
  const removeValue = async (itemId, index) => {
    ...
    setAddData(addData.filter((item) => {
  ...
  const rightSwiper = (item, index) => {
    ...
    return (
      <TouchableOpacity onPress={() => removeValue(item, index)}
        style={styles.rightSwiper}>
        <MaterialIcons style={styles.plusIcon} name={'delete-outline'} size={20} color={"#fff"} />
      </TouchableOpacity>
    )
  }
  ...
  return (
    <View style={styles.containt}>
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        <TopnavBar />
        <HistoryCom month={month} setMonth={(month) => setMonth(month)} expenseTotal={expenseTotal} incomeTotal={incomeTotal}
          incomePress={() => setInSelect('income')}
          expensePress={() => setInSelect('expense')} />

        {
          addData == '' ? <View style={styles.noRecors}>
                  ...
                  <Swipeable
                    ...
                    <View style={styles.budgetContaint}>
                      ...
                      <View style={{ flex: 1, marginRight: 10, marginLeft: 10 }}>
                        <Text style={styles.budgetText}>{item.category ? item.category.title : ''}</Text>
                        {inSelect == "expense" ?
                          <Text style={styles.budgetText}>Limits: ADD  {item.limit}</Text> : null
                        }
                        <Text>Spand : ADD {item.amount}.00</Text>

                        {item.limit && <Text style={styles.budgetText}>Remaining : {item.limit - item.amount} </Text>}
                        {inSelect == 'income' ? <View style={styles.budgetborder}></View>
      ...
      <TouchableOpacity onPress={() => navigation.navigate("Calculator")}>
        <AntDesign style={styles.plusIcon} name={'pluscircle'} size={40} color={"#000"} />
      </TouchableOpacity>
    </View>
  )
}
